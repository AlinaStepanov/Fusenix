...
from services_config import fetch_all_services, fetch_service_health, fetch_service_timeline, load_config
from discovery import run_discovery, save_discovery
...
async def sources_status():
    ...
    return {
        ...
        "github": {
            ...
            "repos": [
                {
                    "repo": r.split(":")[0].strip(),
                    "branches": r.split(":")[1].split("|") if ":" in r else None,
                }
                for r in os.environ.get("GITHUB_REPOS", "").split(",") if r.strip()
            ],
        },
        "grafana": {
...
@app.get("/github/deploy-diff")
@limiter.limit("30/minute")
async def github_deploy_diff(
    request: Request,
    owner: str,
    repo: str,
    ref: str,
    ...
    """Return commits and recently merged PRs for a GitHub deployment."""
    token = os.environ.get("GITHUB_TOKEN")
    if not token:
    ...
    async with httpx.AsyncClient(headers=headers, timeout=15) as client:
        commits: list[dict] = []
        compare_url = f"https://github.com/{owner}/{repo}/commits/{ref}"

        if prev_ref and prev_ref != ref:
            compare_url = f"https://github.com/{owner}/{repo}/compare/{prev_ref}...{ref}"
            try:
                resp = await client.get(
                    f"https://api.github.com/repos/{owner}/{repo}/compare/{prev_ref}...{ref}",
                )
                if resp.status_code == 200:
                    data = resp.json()
                    commits = [
                        {
                            "sha": c["sha"][:7],
                            "message": c["commit"]["message"].split(chr(10))[0][:100],
        ...
        try:
            ...
            logger.warning("GitHub PRs fetch failed: %s", exc)
...
async def get_services(request: Request):
    """
    Return the full service catalog with live health data from all connectors.
    Reads service definitions from services.yml and enriches each with:
      - CloudWatch alarm states
      - Grafana active alerts
      - PagerDuty active incidents
      - GitHub last deployment
    """
    cw      = get_cloudwatch()
    grafana = get_grafana()
    pd      = get_pagerduty()
    gh      = get_github()
    result  = await fetch_all_services(cw, grafana, pd, gh)
    return result


@app.get("/services/{service_id}")
@limiter.limit("60/minute")
async def get_service_detail(request: Request, service_id: str):
    """Return live health detail for a single service by its config id."""
    config   = load_config()
    svc_cfgs = config.get("services", [])
    svc_cfg  = next((s for s in svc_cfgs if s.get("id") == service_id), None)
    if not svc_cfg:
        raise HTTPException(status_code=404, detail=f"Service '{service_id}' not found in services.yml")

    cw      = get_cloudwatch()
    grafana = get_grafana()
    pd      = get_pagerduty()
    gh      = get_github()
    result  = await fetch_service_health(svc_cfg, cw, grafana, pd, gh)
    return result


# ── Service timeline (Level 2 drill-down) ────────────────────────────────────

@app.get("/services/{service_id}/timeline")
@limiter.limit("30/minute")
async def get_service_timeline(
    ...
    sources (CloudWatch alarm_prefix, GitHub repo, PagerDuty service_id,
    Grafana alerts matching service name) plus cross-service correlation context
    (other services with critical/warning events in the same time window).
    """
    config   = load_config()
    svc_cfgs = config.get("services", [])
    svc_cfg  = next((s for s in svc_cfgs if s.get("id") == service_id), None)
    ...
    result = await fetch_service_timeline(
        svc_cfg      = svc_cfg,
        all_svc_cfgs = svc_cfgs,
        cw           = cw,
        grafana      = grafana,
        pd           = pd,
        gh           = gh,
...
async def get_oncall(request: Request):
    """
    Return current on-call assignments from PagerDuty.
    Returns {"oncall": [...], "configured": bool}
    """
    pd = get_pagerduty()
    if not pd.api_key:
        return {"configured": False, "oncall": []}
    try:
        oncall = await pd.get_oncall()
        return {"configured": True, "oncall": oncall}
    except Exception as exc:
        logger.error("Oncall fetch error: %s", exc)
        return {"configured": False, "oncall": [], "error": str(exc)}
...
async def get_active_incidents(request: Request):
    """
    Return all currently triggered or acknowledged incidents from PagerDuty.
    Returns {"incidents": [...], "count": int, "configured": bool}
    """
    pd = get_pagerduty()
    if not pd.api_key:
        return {"configured": False, "incidents": [], "count": 0}
    try:
        incidents = await pd.get_active_incidents()
        return {"configured": True, "incidents": incidents, "count": len(incidents)}
    except Exception as exc:
        logger.error("Active incidents fetch error: %s", exc)
        return {"configured": False, "incidents": [], "count": 0, "error": str(exc)}
...
async def discover_services(request: Request):
    """
    Auto-discover services from connected APIs (CloudWatch, PagerDuty, Grafana, GitHub).
    Returns { raw, suggested_services, yaml, summary }
    No side effects — does NOT modify services.yml.
    """
    result = await run_discovery()
    return result


@app.post("/discover/save")
@limiter.limit("5/minute")
async def save_discovered_services(request: Request, body: SaveDiscoveryRequest):
    """
    Write the supplied YAML to services.yml (backs up the existing file first).
    Returns { path, backup_path, service_count, bytes_written }
    """
    if not body.yaml or not body.yaml.strip():
        raise HTTPException(status_code=400, detail="yaml field is required and must not be empty")
    result = save_discovery(body.yaml)
    return result
