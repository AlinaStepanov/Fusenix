...
GitHub connector — fetches per repo:
  1. Commits pushed in the time window (branches: configurable per repo, defaults to main/master)
...
class GitHubConnector:
    def __init__(self, token: Optional[str], repos: list[str]):
        # Parse "owner/repo" or "owner/repo:branch1|branch2" entries.
        # Branches after ":" are "|"-separated; omitting ":" means auto-detect
        # (tries "main" then "master", stops on first success).
        self._repo_configs: list[tuple[str, list[str] | None]] = []
        for r in repos:
            r = r.strip()
            if not r:
                continue
            if ":" in r:
                repo_part, branches_raw = r.split(":", 1)
                branches: list[str] | None = [
                    b.strip() for b in branches_raw.split("|") if b.strip()
                ] or None
            else:
                repo_part = r
                branches = None  # auto-detect main/master
            self._repo_configs.append((repo_part.strip(), branches))
    ...
    async def fetch(self, start: datetime, end: datetime) -> list:
        ...
        async with httpx.AsyncClient(headers=self.headers, timeout=20) as client:
            tasks = [
                self._fetch_repo(client, *self._split_repo(repo), start, end, branches)
                for repo, branches in self._repo_configs
                if self._split_repo(repo)[0]
            ]
            results = await asyncio.gather(*tasks, return_exceptions=True)
            for (repo, _), result in zip(self._repo_configs, results):
    ...
    async def _fetch_repo(self, client, owner, name, start, end, branches=None) -> list:
        results = await asyncio.gather(
            self._fetch_commits(client, owner, name, start, end, branches),
    ...
    async def _fetch_commits(self, client, owner, name, start, end, branches=None) -> list:
        TimelineEvent = _get_event_model()
        events = []
        # Use explicitly configured branches, or auto-detect main → master.
        branches_to_try = branches if branches else ["main", "master"]
        auto_detect     = not branches
        ...
        for branch in branches_to_try:
            try:
                ...
                if resp.status_code == 422:
                    continue   # branch does not exist, try next
