...
class CloudWatchConnector:
    ...
    def _fetch_alarms(self, start: datetime, end: datetime) -> list:
        """Fetch alarm state history for the requested window.

        Also fetches a 7-day look-back in the same API call so we can annotate
        each event with a recurrence_7d count (how many times the alarm fired in
        the past week) — no extra API calls required.
        """
        from datetime import timedelta
        TimelineEvent = _get_event_model()
        events = []
        try:
            # Extend the fetch window back 7 days so we can compute recurrence counts.
            ...
            if self.alarm_prefix:
                ...
                for name in alarm_names:
                    ...
                    for page in paginator.paginate(
                        ...
                        items_all.extend(page.get("AlarmHistoryItems", []))
            else:
                paginator = self._cw.get_paginator("describe_alarm_history")
                items_all = []
                for page in paginator.paginate(
            ...
            # Build per-alarm ALARM-state-transition count for the 7-day window.
            recurrence_7d: dict[str, int] = {}
            for item in items_all:
                summary = item.get("HistorySummary", "").lower()
                if " to alarm" in summary or summary.endswith("alarm"):
                    n = item["AlarmName"]
                    recurrence_7d[n] = recurrence_7d.get(n, 0) + 1

            # Filter to the user-requested window for timeline display.
            ...
            items = [
                ...
                if start_tz <= (
                    i["Timestamp"] if i["Timestamp"].tzinfo
                    else i["Timestamp"].replace(tzinfo=timezone.utc)
                ) <= end_tz
            ]
            ...
            for item in items:
                ...
                rec       = recurrence_7d.get(alarm_name, 0)

                events.append(TimelineEvent(
                    id=self._uid("cw_alarm", alarm_name, str(item["Timestamp"])),
                    source="cloudwatch",
                    ...
                    url=(
                        ...
                        f"?region={self.region}#alarmsV2:alarm/{alarm_name}"
                    ),
                    raw={
